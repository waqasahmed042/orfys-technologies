...
const Blogs: React.FC = () => {
    ...
    return (
        <section className="relative w-full flex items-center mt-28 mb-12 overflow-hidden" style={{ backgroundColor: "var(--bg-primary)" }}>
            <div className="w-full max-w-[1400px] mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <main className="lg:col-span-8 space-y-6">
                    {currentPosts.length > 0 ? (
                                ...
                                <div className="w-full md:w-2/3 flex flex-col">
                                    ...
                                    <div className="flex flex-wrap gap-2 items-center pt-4 border-t border-[var(--border-default)]">
                                        ...
                                        {post.tags.map((tag, idx) => (
                                            <span key={idx} onClick={() => { setSearchTerm(tag); setCurrentPage(1); }} className="text-xs text-[var(--accent-primary)] hover:underline cursor-pointer">
                        ...
                        /* Show only when results are empty */
                        <div className="text-center py-20 text-gray-500 font-medium">No blogs found for this tag.</div>
                    )}

                    {/* Pagination - only shows if results exist and there's more than 1 page */}
                    {totalPages > 1 && (
                        <div className="py-8">
                            <nav className="flex items-center justify-between pt-4 md:pt-5" aria-label="Pagination">
                                <div className="flex flex-1">
                                    <button
                                        onClick={() => handlePageChange(currentPage - 1)}
                                        disabled={currentPage === 1}
                                        className={`flex items-center gap-x-1.5 text-sm font-semibold border border-transparent rounded-lg px-3 py-2 transition-colors ${currentPage === 1 ? 'cursor-not-allowed opacity-30' : 'cursor-pointer hover:text-[var(--accent-primary)] hover:border-[var(--accent-primary)]'}`}
                                    >
                                        <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none"><path d="M15 6L9 12L15 18" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" /></svg>
                                        Previous
                                    </button>
                                </div>
                                ...
                                <div className="hidden md:flex md:items-center gap-2">
                                    {getPagination().map((page, idx) =>
                                        page === '...' ? (
                                            <span key={idx} className="inline-flex h-10 w-10 items-center justify-center text-sm font-medium text-gray-500">...</span>
                                        ) : (
                                            <button
                                                key={idx}
                                                onClick={() => handlePageChange(Number(page))}
                                                className={`inline-flex h-10 w-10 items-center justify-center rounded-lg text-sm cursor-pointer font-medium transition-all ${currentPage === page ? "bg-[var(--accent-primary)] text-white" : "border border-gray-300 text-gray-500 hover:bg-gray-50"}`}
                                            >
                                                {page}
                                            </button>
                                        )
                                ...
                                <div className="flex flex-1 justify-end">
                                    <button
                                        ...
                                        className={`flex items-center gap-x-1.5 text-sm font-semibold border border-transparent rounded-lg px-3 py-2 transition-colors ${currentPage === totalPages ? 'cursor-not-allowed opacity-30' : 'cursor-pointer hover:text-[var(--accent-primary)] hover:border-[var(--accent-primary)]'}`}
                                    >
                                        Next
                                        <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none"><path d="M9 6L15 12L9 18" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" /></svg>
                                    </button>
                                </div>
                            </nav>
                        </div>
                ...
                <aside className="lg:col-span-4 space-y-8">
                    <div className="relative">
                        <input
                            ...
                            className="w-full p-4 pr-12 border border-[var(--border-default)] rounded-xl text-[var(--text-primary)] placeholder:text-slate-400 focus:outline-none focus:ring-0 transition-all"
                        />
                    ...
                    <div className="bg-[var(--accent-primary)] text-white p-8 rounded-2xl shadow-xl text-center relative overflow-hidden">
                        <div className="relative z-10">
                            ...
                            <p className="text-sm opacity-90 mb-6">SCAND’s the company to call for smart solutions and easy-going consulting.</p>
                            <Link href="/contact-us" className="inline-block w-full sm:w-auto">
                                <button className="w-full bg-white text-[var(--accent-primary)] cursor-pointer font-semibold py-3 rounded-xl hover:bg-slate-100 transition-colors shadow-lg px-6">Contact us</button>
                            </Link>
                        </div>
                    </div>
                    ...
                    <div className="border border-[var(--border-default)] bg-[var(--bg-primary)] rounded-2xl overflow-hidden shadow-sm">
                        <div className="p-5 border-b border-[var(--border-default)] font-bold text-[var(--text-primary)] flex justify-between items-center">
                            Categories
                            <svg className="w-4 h-4 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
                        </div>
                        <ul className="divide-y divide-[var(--border-default)]">
                            {categories.map((cat, i) => (
                                <li key={i} className="p-4 flex justify-between text-sm text-[var(--text-secondary)] hover:bg-slate-50 dark:hover:bg-slate-800/50 cursor-pointer transition-colors group">
                                    <span className="group-hover:text-[var(--accent-primary)] transition-colors">{cat.name}</span>
